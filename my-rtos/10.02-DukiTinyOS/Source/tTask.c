...
void tTaskInit(tTask *tTask, void(*entry)(void *), void * param, uint32_t prio, tTaskStack * tTaskStack) {
    ...
    *(--tTaskStack) = (unsigned long)(1 << 24); 

    // 设置PC寄存器（程序计数器），即任务入口地址
    *(--tTaskStack) = (unsigned long)entry;     

    // 设置LR寄存器（链接寄存器），即任务返回地址
    *(--tTaskStack) = (unsigned long)0x14;      

    // 设置其他寄存器的初始值
    *(--tTaskStack) = (unsigned long)0x12;      // R12寄存器
    *(--tTaskStack) = (unsigned long)0x3;       // R3寄存器
    *(--tTaskStack) = (unsigned long)0x2;       // R2寄存器
    *(--tTaskStack) = (unsigned long)0x1;       // R1寄存器
    *(--tTaskStack) = (unsigned long)param;     // 任务的入口函数参数

    // 设置其他寄存器的初始值
    *(--tTaskStack) = (unsigned long)0x11;      // R11寄存器
    *(--tTaskStack) = (unsigned long)0x10;      // R10寄存器
    *(--tTaskStack) = (unsigned long)0x9;       // R9寄存器
    *(--tTaskStack) = (unsigned long)0x8;       // R8寄存器
    *(--tTaskStack) = (unsigned long)0x7;       // R7寄存器
    *(--tTaskStack) = (unsigned long)0x6;       // R6寄存器
    *(--tTaskStack) = (unsigned long)0x5;       // R5寄存器
    *(--tTaskStack) = (unsigned long)0x4;       // R4寄存器
